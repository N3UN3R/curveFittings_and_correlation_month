...
def fitting_and_plotting(sorted_producer_numbers,sorted_price_means,producer_number, reached_price):
    ...
    #set title
    plt.title('mod. Kruskal: Producer-Haushalte und erreichter Strompreis')
    ...
    fig.savefig("legende_producer_price_scalable.svg")
    fig.savefig("legende_producer_price_scalable.pdf")

     #legende
    from matplotlib.lines import Line2D
    legend_elements = [Line2D([0], [0], color='red', label='durchschnittlich erreichter Strompreis'),
                       Line2D([0], [0], color='black', label='Approximationskurve'),
                       Line2D([0], [0], color='w', marker='o', label='erreichter Strompreis',markerfacecolor='deepskyblue',markersize=5)]
