...
def correlation_producers_runtime(sorted_producer_numbers,sorted_runtime_means,producer_number, runtime):
    ...
    print("p-value")
    print(p)
    #print("R2", r2_score(sortedDict_keys_transformed, slope*producers+intercept))
    print("r-squared:", r ** 2)
    ...
    plt.title('mod. Kruskal: Producer-Haushalte und Laufzeit')
    ...
    legend_elements = [Line2D([0], [0], color='red', label='durchschnittliche Laufzeit'),
                         Line2D([0], [0], color='black', label='Regressionsgerade'),
                         Line2D([0], [0], color='w', marker='o', label='Laufzeit', markerfacecolor='deepskyblue',
                        markersize=5)]
    ...
    #fig.savefig("legende_orrelation_producer_runtime_scalable.pdf")
    fig.savefig("legende_korrelation_producer_runtime_scalable.svg")
